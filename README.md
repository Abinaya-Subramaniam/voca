...
- **Agentic Voca Bot** — an AI agent that doesn't just answer questions;
  it can read a person's real communication data and *act* on it directly
...
- Native `IntersectionObserver` for scroll-based UI, Web Speech API for
  text-to-speech — no extra runtime dependencies for either
...
- **`board` scope** — issued on `/api/auth/kid-login` (username + 4-digit
  ...
  board, journal, and their own profile — no access to caregiver routes,
...
| `OPENAI_API_KEY` | — | Primary LLM provider for the AI Companion and weekly coach |
| `OPENAI_MODEL` | `gpt-4o-mini` | OpenAI model name |
| `GEMINI_API_KEY` | — | Optional automatic fallback if OpenAI errors out |
...
- The only data that ever leaves the device is an **anonymized** summary of
  communication patterns (word counts, categories, timing — no names, no
