...
export default function JournalView() {
  ...
  useEffect(() => {
    if (activeProfileId) {
      getJournalEntries(activeProfileId).then(setEntries).catch(() => setEntries([]))
    }
  }, [activeProfileId])
  ...
  async function handleSaveEntry() {
    ...
    const finalSentences = currentSentence.length > 0
      ...
      : sentences
    await saveJournalEntry(activeProfileId, {
      moodSymbol: selectedMood,
      sentences: finalSentences,
    })
    setEntries(await getJournalEntries(activeProfileId))
    setScreen('list')
  }

  async function handleDelete(entryId) {
    if (!confirm('Delete this journal entry?')) return
    await deleteJournalEntry(activeProfileId, entryId)
    setEntries(await getJournalEntries(activeProfileId))
    setViewEntry(null)
    setScreen('list')
  }
